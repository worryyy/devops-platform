...
> ③ CI/CD 各阶段耗时分布及各优化项的量化贡献；④ 数字的准确口径与无歧义的表述方式。
...
**三个必改点**（不修改则与实测事实不符，存在被追问后无法自洽的风险）：
1. "BuildKit **Registry** 层缓存" → 实际是 **PVC 本地**层缓存（ACR 个人版拒绝 buildkit cacheconfig 清单类型，实测失败后调整方案）
...
**核心结论**：冷态瓶颈为逐服务依赖编译，热态瓶颈转为仓库克隆——优化后期的主要成本不在构建而在源码获取。据此追加浅克隆优化（`--depth 20`，预计热路径再减少约 60-70s，未单独量化）。
...
关键结论：**服务间共享带来的收益大于缓存命中本身**——第一个服务完成依赖编译后，其余 12 个服务直接复用，此项贡献 71%；跨构建预热贡献次之；影响分析对日常体验改善最明显（13→1），但在全量口径下占 4%。
...
**配套答法**（对应"全量构建也仅 3.4 分钟，影响分析的意义是什么"类追问）：
...
| （无 selfHeal 竞态内容） | **实测发现 Argo selfHeal 会在补偿窗口把失败版本重新拉回集群**，加了 pause/resume 防护并验证 | **必加**——超出原设计的真实发现，为本轮实测最有价值的新增内容 |
...
| "SLI 仅保留必要维度" | ✅ 前置问题经实测发现：KSM 注解白名单键名大小写（AllowList/Allowlist）导致关联断链，修复后 release_info 序列正常生成 | 可留；该问题可作为实现细节补充 |
