...
> **状态**：✅ 已完成（2026-09-21；A3 全量发布链与 A4 revert 演练待真实发布窗口补验，其余验收通过）

## 1. 任务清单

### 1.1 凭据与配置
- [x] Jenkins 生成 API token（用户设置页），写入 Secret
      `delivery/jenkins-api-token`（平台经 envFrom 读取）；
      `k3s/secrets/` 增 example 模板
- [x] `internal/config/config.go` 增加：`JENKINS_URL`
...
### 1.2 后端 delivery 模块
- [x] `internal/delivery/jenkins.go`：Jenkins 客户端（纯 HTTP）：
...
- [x] `migrations/004_pipeline_runs.sql`：
...
- [x] `internal/delivery/` handler 与 service：
...
- [x] `internal/delivery/argocd.go`：in-cluster clientset 读
      Application status（sync revision/health），发布详情页展示
      「GitOps 同步状态」卡片

### 1.4 前端
- [x] `/pipelines` 页：触发表单（服务多选来自 services 表、SHA 对填一
      个或默认 HEAD）、运行列表（状态徽标：queued/running/success/
      failed）、行点开 stage 时间线（横向 stepper，耗时标注）
- [x] `/pipelines/:id` 详情：GitOps PR 链接、Argo 状态卡、digest、
      release 记录时间线（releasing→stable/failed）；失败态显示
      「一键 Revert」按钮（确认弹窗 → POST revert → 展示 PR 链接）
- [x] `/services/:name` 详情页加「最近发布」区块（复用列表组件）

### 1.5 测试
- [x] jenkins 客户端单测（httptest 模拟 Jenkins API）
- [x] webhook handler 单测（secret 鉴权、stages 合并）
- [x] 契约测试更新：Jenkinsfile 断言新增 `PLATFORM_WEBHOOK_URL`/
      `Notify platform` stage 存在

## 2. 验收标准
- [x] A1 触发：页面选 `theme` 触发构建 → pipeline_runs 出现记录且
      Jenkins 对应 build 启动（`Jenkins 构建队列可见`）
- [x] A2 进度：构建期间详情页 stage 时间线随 webhook 推进更新（≤30s
      延迟）；构建结束状态与 Jenkins 一致
- [ ] A3 发布全流程（SKIP_RELEASE 构建链已通；全量发布待补）：页面触发 → 合并 → Argo 同步 → wait-for-release
      验证 → 详情页显示 stable + digest 与 `service_releases` 一致
- [ ] A4 一键 revert（API/权限/409 边界已测；真实失败发布演练待补）：人为让一次发布失败（如注入坏 tag），详情页点
      Revert → GitHub 出现 `revert/theme/<build>` PR，人工合并后集群
      回到上一版且 `service_releases` 记录链完整
- [x] A5 权限：viewer 角色触发/回退按钮不可用（403），admin 可用
